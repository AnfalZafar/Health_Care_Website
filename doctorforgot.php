...
function sendmail($email , $change_pass){
    ...
    try {
        ...
        <h3>   
        <a href='http://localhost/health/doctor-change-pass.php?emailId=$email&change_Id=$change_pass' >Reset Your Password</a>    
        </h3>
        ";
         $mail->send();
...
if(isset($_POST["reset"])){
    $email = $_POST["d_email"];
    $select = "SELECT * FROM `doctors_table` WHERE `doctor_gmail` = '$email' ";
    $run = mysqli_query($connect , $select);
    $num_rows = mysqli_num_rows($run);
    $fetch = mysqli_fetch_array($run);
    if($num_rows != 0){
      if($fetch["doctor_gmail"] == $email){
      $change_pass = bin2hex(random_bytes(16));
      $update = "UPDATE `doctors_table` SET `reset_code`='$change_pass' WHERE `doctor_gmail` = '$email' ";
    ...
    <div class="main">
        ...
        <div class="form">
            ...
            <form method="post">
                ...
                <div class="back">
                    <h6>Go Bake to <a href="doctorsignin.php">Sign In</a></h6>
                </div>
