...
if(isset($_POST["change"])){
   $pass_1 = $_POST["pass_1"];
   $select = "SELECT * FROM `sign_table` WHERE `sign_email` = '$email' AND `reset_code` = '$change_pass' ";
   $run = mysqli_query($connect , $select);
   $num_rows = mysqli_num_rows($run);
   $fetch = mysqli_fetch_array($run);
   if($num_rows != 0){
    $update = "UPDATE `sign_table` SET `sign_pass`='$pass_1' WHERE `sign_email` = '$email' AND `reset_code` = '$change_pass' ";
