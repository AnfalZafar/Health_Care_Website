...
function sendmail($email , $change_pass){
    ...
    try {
        ...
        $mail->Subject = 'Email Verifiction from Health Care';
        $mail->Body    = "Click below the link to Registration<br>
        <h3>   
        <a href='http://localhost/health/change-pass.php?emailId=$email&change_Id=$change_pass' >Reset Your Password</a>    
        </h3>
        ";
         $mail->send();
...
if(isset($_POST["reset"])){
    $email = $_POST["c_email"];
    $select = "SELECT * FROM `sign_table` WHERE `sign_email` = '$email' ";
    $run = mysqli_query($connect , $select);
    $num_rows = mysqli_num_rows($run);
    $fetch = mysqli_fetch_array($run);
    if($num_rows != 0){
      if($fetch["sign_email"] == $email){
      $change_pass = bin2hex(random_bytes(16));
      $update = "UPDATE `sign_table` SET `reset_code`='$change_pass' WHERE `sign_email` = '$email' ";
      if(mysqli_query($connect , $update) && sendmail($email , $change_pass)){
        $msg = "<div class='alert alert-danger' role='alert'>
        Rest code is send to your Email
    </div>";
      }
      }
...
<body>
    ...
    <div class="main">
        ...
        <div class="form">
            ...
            <form method="post">
                <div class="name">
                    <input type="email" name="c_email" placeholder="Enter Email">
                </div>
                <div class="btn">
                    <button type="submit" name="reset">Send Resent Link</button>
                </div>
                <div class="back">
                    <h6>Go Bake to <a href="signin.php">Sign In</a></h6>
                </div>
